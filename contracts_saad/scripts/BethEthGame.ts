...
let betEthGameAddress: Address;
let bethEthTokenAddress: Address;
...
async function initContracts() {
    const contract = await viem.deployContract("betEthGame", [
        "LotteryToken",
        "LT0",
        TOKEN_RATIO,
        parseEther(BET_PRICE),
    ]);
    betEthGameAddress = contract.address;
    bethEthTokenAddress = await contract.read.bethEthToken();
}
...
async function checkState() {
    const contract = await viem.getContractAt("Lottery", betEthGameAddress);
...
async function bet(index: string, amount: string) {
    ...
    const token = await viem.getContractAt("LotteryToken", tokenAddress);
    const contract = await viem.getContractAt("Lottery", betEthGameAddress);
    const allowTx = await token.write.approve([betEthGameAddress, MAXUINT256], {
        account: accounts[Number(index)].account,
    });
